...
| expo-linear-gradient | ~57.0.1 (seit PR #193 installiert, seit PR #195 genutzt: CategoryTabBar/Button) |
| expo-haptics         | ~57.0.2 (seit PR #195 – Impact-Feedback bei Button-Press/Drag&Drop)             |
| AsyncStorage         | ^3.1.1                                                                          |
...
**Offen nach vc17 – erledigt in vc18 (2026-08-30):** `android:resizeableActivity="true"` (Issue #202) fehlte im vc17-Artefakt, weil `patch-twa-target-sdk36.sh` beim Build nicht mitlief. In vc18 nachgezogen und am Artefakt verifiziert (s. o.).
...
## Letzte Merges / Fixes (2026-09-16)

| Was                                                                  | Wann       | Details                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------------------------------------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Issue #280 – RN-0.87-Blocker für PR #279 aufgelöst**               | 2026-09-16 | ✅ Root Cause bestätigt: `jest-expo` ist auf SDK-57-Stand (`~57.0.5`, letzte 57.x-Version) und kennt `@react-native/assets-registry/registry` unter `react-native@0.87.1` nicht mehr – ein kompatibles `jest-expo`-Update existiert nur mit SDK 58 (Major-Sprung, hier nicht angegangen). Statt auf das Dependabot-Gruppen-Update zu warten, die 15 unkritischen Bumps aus PR #279 einzeln übernommen (`expo` 57.0.21, `expo-linking`/`expo-router` Patch-Bumps, `react`/`react-dom` 19.3.0, `react-native-safe-area-context` 5.9.1, `react-native-screens` 4.27.0, `react-native-svg` 15.15.5, `zod` 4.6.1, `@types/react` 19.3.0, `@typescript-eslint/eslint-plugin` 8.70.0, `eslint` 10.10.0, `prettier` 3.9.6) – `react-native`/`@react-native/jest-preset` bewusst auf `0.86.3` zurückgehalten (per `npm view react-native@0.86.3 peerDependencies` verifiziert: `react ^19.2.3` erlaubt `19.3.0`, kein Konflikt). Dabei `react-test-renderer` neu explizit als devDependency (`19.3.0`, exakt zur React-Version) ergänzt – ohne das blieb es über `@testing-library/react-native` auf `19.2.3` hängen und `ensurePeerDeps` brach alle 17 Komponententests ab. Zusätzlich die in #280 gemeldete Prettier-Drift in `CLAUDE.md` (fehlender Newline am Dateiende) gefixt. `tsc --noEmit` sauber, 384/384 Tests grün, ESLint 0 Warnings, `expo export --platform web` erfolgreich.                                           |
| **PR #269 (#268):** Code-Health-Audit – CI-Gates geschärft           | 2026-09-06 | ✅ testing: `tsc --noEmit \|\| true` in `ci-cd.yml` entfernt (`tsc` lief zum Zeitpunkt des Fixes bereits sauber auf 0 Fehler) – Gate ist jetzt scharf. `pull_request`-Trigger um `testing` ergänzt (lief zuvor nur gegen `main`, PRs gegen `testing` wurden laut Branch-Strategie nie von CI geprüft, siehe PR #209/#214). `npm test` in CI läuft jetzt mit `--coverage`, damit `coverageThreshold` (Issue #231) tatsächlich greift statt nie ausgewertet zu werden. Bundle-Size-Check hat jetzt einen Schwellwert (13.000 KB, aktuell ~6,4 MB). `src/services/storage.ts`: `setGuestMode`/`isGuestMode`/`clearAll` auf `withStorageError`/neues `withStorageErrorFallback` (`storageError.ts`) umgestellt – `savePlants`/`loadPlants`/`importPlants` bewusst unverändert gelassen, da sie Fehler weiterreichen (rethrow/`STORAGE_CORRUPTED`) statt sie zu schlucken, ein fremdes Verhalten für die bestehende Utility. `ErrorBoundary.tsx` behält bewusst eine eigene hartkodierte Palette (jetzt kommentiert, warum: rendert außerhalb von Theme-Providern). Totes `monthHelper.ts`: `MONTH_NAMES`, `HALF_MONTH_NAMES`, `halfMonthToString`, `halfMonthRangeToString` entfernt (nirgends im App-Code referenziert), zugehörige Tests entfernt (384 → 371 Tests). `tsc` 0, 371/371 Tests grün, `eslint`/`prettier --check` sauber (ausstehender `CLAUDE.md`-Formatierungswarn ist vorbestehend, nicht Teil dieser Änderung). |
| **Sync: main → testing**                                             | 2026-09-04 | ✅ testing: main und testing waren divergiert (main hatte 7 eigene Dependabot-Bumps aus #237–#245 s. u., testing hatte eigenständig die ESLint-9→10-Migration PR #251 gemacht, schließt Meta-Issue #249) – Konflikte in `CLAUDE.md`/`package.json`/`package-lock.json`. Aufgelöst: testings neuere ESLint-Toolchain behalten (`eslint` ^10.9.1, `@eslint/js` ^10.0.1, `@typescript-eslint/*` ^8.69.0), `jest` 30.5.0 + `@types/jest` 30.0.0 von main übernommen, `package-lock.json` neu generiert. Damit sind die 3 zuvor als „nicht gemergt“ dokumentierten Dependabot-PRs #243/#246/#247 aus der #237–#245-Reihe obsolet – ihr Inhalt (ESLint 10) steckt bereits in #251.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **PRs #237–#245:** Dependabot-Bulk-Merge (7 von 10 offenen PRs)      | 2026-09-03 | ✅ main: 7 grüne Dependabot-PRs gemergt (squash) – reine CI/Test-Tooling-Bumps ohne Breaking Changes: `actions/checkout` 4→7 (#241), `actions/setup-node` 4→7 (#239), `reusable-security-scan.yml` 1→2 (#240), `reusable-gitignore-audit.yml` 1→2 (#238), `reusable-dev-standards-audit.yml` 1→2 (#237), `eslint-config-prettier` 9.1.2→10.1.8 (#244), `jest`+`@types/jest`-Bump (#245). Die restlichen 3 offenen Dependabot-PRs (#243, #246, #247) wurden **nicht** gemergt – alle drei ziehen ESLint 9→10 als Major-Bump mit, der `npm run lint` bricht (`eslint-plugin-react` nutzt intern `context.getFilename()`, das ESLint 10 entfernt hat). Root Cause im CI-Log von #247 verifiziert. Für die Migration Meta-Issue **#249** angelegt mit Root-Cause-Analyse und verbindlicher Reihenfolge (Plugin-Kompat prüfen → Migrations-Branch → lokal lint+prettier+tests verifizieren → PR gegen `testing`). Dependabot-PRs waren alle direkt gegen `main` geöffnet (nicht `testing`) – Dependabot-Default, hält sich nicht an die Branch-Strategie des Projekts, war aber unkritisch, da jeder Merge einzeln per CI-Check-Prüfung freigegeben wurde.                                                                                                                                                                                                                                                                         |
| **PR #236:** testing → main (Release, #228/#231/#232/#234)           | 2026-09-02 | ✅ main `63e5fb4`: Release-Merge `testing → main`, 7 Commits nachgezogen. Konflikt in CLAUDE.md (Changelog-Abschnitt, `main` hatte den 2026-08-29-Stand ohne PR #229/#227 noch nicht) und `twa-manifest.template.json` (`appVersionCode` 18 vs. 19) – höherer Stand (19, testing) übernommen. `tsc --noEmit` sauber, 384 Tests grün. Schließt #228 (Dependabot + CodeQL), #231 (`coverageThreshold`), #232 (Pre-Push-Hook/Prettier-Fix auf CLAUDE.md) und #234 (Restdeutsch im UI, Tab-Bar-Lesbarkeit) – alle vier waren bereits vollständig in `testing` umgesetzt, PR #236 hat sie nur formal nach `main` durchgereicht und die Issues geschlossen.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **PR #229 (#228):** CI – Security-Scanning aktivieren                | 2026-08-30 | ✅ testing `fd2cfb9`: Dependabot-Alerts (waren serverseitig deaktiviert) reaktiviert; `codeql.yml`-Versuch (build-mode: none) wieder zurückgezogen, da GitHubs Code-Scanning Default Setup im Repo bereits aktiv ist.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **PR #227:** Play-Console-Empfehlungen Release 18 (R8 + Ausrichtung) | 2026-08-30 | ✅ testing `207e156`: Play Console meldete für vc18/1.6.1 vier Empfehlungen (Edge-to-Edge ×2, Größenänderung/Ausrichtung, R8-Optimierung). Edge-to-Edge war bereits über `patch-twa-edge-to-edge.sh` (#206) abgedeckt. Neu: `scripts/patch-twa-r8-optimization.sh` (setzt `minifyEnabled`/`shrinkResources` im release-BuildType); `patch-twa-target-sdk36.sh` um Schritt 4 ergänzt (normalisiert ein hartes `android:screenOrientation` auf der LauncherActivity auf `unspecified`); `appVersionCode` 18 → 19 für den nächsten Upload. Branch war ursprünglich von `main` statt `testing` abgezweigt und musste vor dem Merge per `git merge origin/testing` nachgezogen werden (Konflikte in CLAUDE.md/twa-manifest.template.json, aufgelöst). Rein Build-Vorbereitung – das `android/`-Projekt selbst (Bubblewrap-generiert, gitignored) konnte in der Remote-Session nicht gebaut/verifiziert werden; Build/Upload/Play-Console-Verifikation bleiben offen.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Issue #222:** Expo SDK 56 → 57 – Security-Findings reduziert       | 2026-08-29 | ✅ testing: `npm audit` vorher 0 critical/10 high/11 moderate, danach **0 critical/0 high/10 moderate** (`npm audit fix`, ohne `--force`). Root Cause war die von Expo/Metro vorgegebene transitive Toolchain (`@expo/metro`, `metro`, `postcss`, `js-yaml`, `brace-expansion`, `nanoid`, `shell-quote` u. a.) – Einzel-Bumps griffen nicht dauerhaft. Bump auf Bundled-Versionen aus SDK 57 (`bundledNativeModules.json` von `expo@57.0.18`, da `npx expo install` durch die Sandbox-Netzwerkpolicy an `api.expo.dev` blockiert war): `expo ^57.0.18`, `expo-router ~57.0.17`, `react-native 0.86.3`, `react-native-web ~0.21.0`, `react-native-screens ~4.26.0`, `expo-constants/-haptics/-linear-gradient/-linking/-navigation-bar/-status-bar` auf ~57.x, `jest-expo ~57.0.5`, `@react-native/jest-preset ^0.86.3`. `tsc --noEmit` sauber, `eslint`/`prettier --check` sauber, 384 Tests grün, `expo export --platform web` + `add-pwa-meta-tags.js`/`add-splash-screen.js` erfolgreich. TWA-Kette (`twa-manifest.template.json`, `assetlinks.json`, Patch-Skripte) unverändert – nicht von der JS-Toolchain betroffen, wie in #222 vorgezeichnet.                                                                                                                                                                                                                                                                        |
