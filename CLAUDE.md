...
| react-native-svg                       | ^15.15.5 (QR-Rendering)                     |
| qrcode-generator                       | ^1.4.4 (pure JS, QR-Matrix, seit PR #172)   |
...
**Update 2026-06-08:** `testing`-Branch existierte **nicht** mehr im Remote (nur `main`, `gh-pages`, `gh-pages-testing`) – frisch aus `origin/main` neu angelegt & gepusht. **PR #172** (Branch `claude/issues-142-171-161-8-0smzi`) gegen `testing` offen: setzt #142 (Drag&Drop), #161 (Pflanzen-Emojis), #171 (Store-Listings), #8 (QR-Teilen) um. 384 Tests grün; 2 Auto-Reviews bearbeitet (Commit `8d0bfef`). Merge-Gate: Label `suggestions gelesen und verstanden` nötig.

- **main branch:** v1.4.0 – Issues #123 + #87 (PR #148 / #149) sowie #8 + #88 (PR #147 / #146, via PR #151 nach main) geschlossen
- **testing branch:** seit 2026-06-08 = `origin/main` (neu angelegt); Basis für PR #172
...
### Emoji/Icon neben Pflanzennamen – getrennte `<Text>`-Knoten (seit PR #172)

Pflanzen-Emojis (`getPlantEmoji` aus `plantEmojis.ts`, Issue #161) **nie** in denselben `<Text>` wie den Namen mischen (`{emoji} {name}` in einem Text). React Native rendert das als kombinierten Text-Knoten („🍅 Tomaten"), wodurch RNTL-Exaktmatches (`getByText('Tomaten')`) brechen. Stattdessen Emoji und Name in **separate `<Text>` in einer Row-`<View>`** legen – so bleibt der Name ein eigener Knoten und Tests matchen weiter. (Ausnahme: die schmale Kalender-Namensspalte in `PlantRowsContainer` darf kombiniert bleiben, da dort kein Test exakt matcht.)

### Drag & Drop von Aktivitätsbalken (Issue #142, PR #172)

`ActivityBar` ist verschiebbar **ohne** `react-native-gesture-handler`/`reanimated` (kein CI-/Bundle-Risiko): **Web** = Maus-Events (`onMouseDown` + window `mousemove`/`mouseup`, `stopPropagation` gegen das Cell-Drag-to-Create darunter), **Native** = `PanResponder` (RN-Core; `onStartShouldSetPanResponder`, Scroll weiter über leere Zellen). Tap (Distanz < `TAP_THRESHOLD`) öffnet den Edit-Dialog. `PanResponder.create()` **muss** in `useRef(...).current` gehalten werden; Callback-Werte (`onPress`/`onMove`/`cellWidth`) über Refs lesen, die **direkt im Render-Body** (nicht via `useEffect`) aktualisiert werden. Der Balken meldet das Delta in _angezeigten_ Einheiten nach oben; `CalendarScreen` rechnet (`isPortrait ? ×4 : ×1`) auf Halbmonate, clamped via `clampActivityShift(start, end, deltaHalfmonths)` auf [0,23] und persistiert via `updateActivity`.
...
- **#142** 🛠️ Drag & Drop für Aktivitäten im Kalender – **PR #172 offen** (gegen `testing`): Balken per Maus (Web) / PanResponder (Native) verschiebbar, Delta → `clampActivityShift` → `updateActivity`
- **#161** 🛠️ Pflanzen-Symbole (Play-Store-Feedback spanisch) – **PR #172**: `plantEmojis.ts`, Anzeige in Pflanzenliste/Kalender/Agenda
- **#171** 🛠️ Store-Eintrag überarbeiten – **PR #172**: mehrsprachige Fastlane-Listings (`fastlane/metadata/android/{de-DE,en-US,es-ES}`)
- **#8** ✅ Template-System: Pflanzpläne teilen und importieren – **in main** (PR #147, via #151); QR-Code-Teilen ergänzt in **PR #172** (`qrcode-generator` + `react-native-svg`)
...
| Was                                                       | Wann       | Details                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **PR #172:** #142 + #161 + #171 + #8-QR (gegen `testing`) | 2026-06-08 | 🛠️ Branch `claude/issues-142-171-161-8-0smzi`; `testing` neu aus `main`. #161 Pflanzen-Emojis (`plantEmojis.ts`, sprachunabhängig) in Pflanzenliste/Kalender/Agenda; #171 mehrsprachige Store-Listings (`fastlane/metadata/android/{de-DE,en-US,es-ES}`); #8 QR-Teilen (`qrcode-generator`, SVG via `react-native-svg`, `buildShareString`); #142 Drag&Drop (Web=Maus / Native=PanResponder, `clampActivityShift`). 384 Tests, Web-Build ok. 2 Auto-Reviews bearbeitet (`8d0bfef`) |
