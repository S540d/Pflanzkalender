...
describe('SettingsScreen', () => {
  ...
  it('renders without crashing', async () => {
    const { root } = await renderWithProviders(<SettingsScreen />);
  ...
  it('renders Dark and System theme buttons', async () => {
    const { getByText } = await renderWithProviders(<SettingsScreen />);
    expect(getByText(/Dark|Dunkel/)).toBeTruthy();
    expect(getByText(/System/)).toBeTruthy();
  });

  it('calls setThemeMode("dark") when Dark button is pressed', async () => {
    const { getByText } = await renderWithProviders(<SettingsScreen />);
    await fireEvent.press(getByText(/^Dark$|^Dunkel$/));
    expect(mockSetThemeMode).toHaveBeenCalledWith('dark');
  });

  it('calls setThemeMode("system") when System button is pressed', async () => {
    const { getByText } = await renderWithProviders(<SettingsScreen />);
    await fireEvent.press(getByText(/^System$/));
  ...
  it('renders English and German language buttons', async () => {
    const { getByText } = await renderWithProviders(<SettingsScreen />);
    expect(getByText('English')).toBeTruthy();
    expect(getByText(/German|Deutsch/)).toBeTruthy();
  });

  it('switches to English when English button is pressed', async () => {
    const { getByText } = await renderWithProviders(<SettingsScreen />);
    await fireEvent.press(getByText('English'));
    expect(getByText('Settings')).toBeTruthy();
  });

  it('switches back to German when Deutsch button is pressed', async () => {
    const { getByText } = await renderWithProviders(<SettingsScreen />);
    // First switch to English
    await fireEvent.press(getByText('English'));
    // Then switch back to German
    await fireEvent.press(getByText('Deutsch'));
  ...
  it('calls sharePlants when export button is pressed', async () => {
    ...
    const { getByText } = await renderWithProviders(<SettingsScreen />);
    await fireEvent.press(getByText(/Export as JSON|Als JSON exportieren/));
  ...
  it('renders feedback and Ko-fi links', async () => {
    const { getByText } = await renderWithProviders(<SettingsScreen />);
    expect(getByText(/Feedback|feedback/i)).toBeTruthy();
    expect(getByText('Ko-fi')).toBeTruthy();
  });

  it('navigates to "/" (not "/(tabs)") when Schließen/Close is pressed', async () => {
    const { getByText } = await renderWithProviders(<SettingsScreen />);
    await fireEvent.press(getByText(/Schließen|Close/));
  ...
  it('shows a native-only alert when import button is pressed on non-web platform', async () => {
    ...
    const { getByText } = await renderWithProviders(<SettingsScreen />);
    await fireEvent.press(getByText(/Aus JSON importieren|Import from JSON/));
    await waitFor(() => {
